...
def build_system_prompt(knowledge_dir: str) -> str:
    ...
    prompt_parts = [
        ...
        "The algorithm uses the Golub-Kahan (TGK) tridiagonal + DSTEMR (MR³) approach.\n",
        "\n## Your Goal\n",
        "Improve the `bidiag_svd()` function to:\n",
        "1. Pass ALL test matrices (STCollection + adversarial) with res ≤ 5, ortho ≤ 5\n",
        "2. Maintain O(n²) worst-case complexity (scaling ratio ≤ 5x when n doubles)\n",
        "3. Handle extreme condition numbers (up to 10^15)\n",
        "\n## Key Constraints\n",
        "- The file is bidiag_svd.h — a single self-contained C++ header\n",
        "- You can call Fortran routines via extern \"C\" (DSTEMR, DBDSGR, DBDSQR, etc.)\n",
        "- O(n³) operations (dense n×n matrix multiply, global MGS) are FORBIDDEN\n",
        "- The evaluation compiles your file with evaluate.cpp and runs test matrices\n",
        "- dbdsgr_ (f2c) requires ftnlen args: pass 1, 1 at end of each call\n",
        "\n## Available Resources\n",
        "- knowledge/ directory: papers, bugs, test matrices, code documentation\n",
        "- src/evaluate.cpp: the C++ evaluation driver (do NOT modify)\n",
        "- src/bidiag_dbdsqr.h: DBDSQR reference (379/379 pass, O(n³))\n",
        "- src/bidiag_hgbsvd.h: HGBSVD reference (174/379 pass, O(n²))\n",
        "- src/bidiag_tgk_stemr.h: TGK+STEMR reference (93/379 pass, O(n²))\n",
        "- Fortran source: ../bidiag-algo/MRRR Resources/Code/tdsolver/xmr/SRC/O/\n",
        "\n## Known Failure Modes\n",
        ...
        "- spike, saw_tooth: degenerate spectrum trips HGBSVD\n",
        "- chkbd: tiny matrices where HGBSVD returns info!=0\n",
        "\n## Key Techniques\n",
        "- One-sided recovery: U = B*V/σ when direct extraction fails\n",
        "- B^TB eigensolver path for well-conditioned matrices (n×n not 2n×2n)\n",
        "- Selective reorthogonalization within clusters (chunked MGS, max 32)\n",
        "- Scale separation: split B at near-zero off-diagonals\n",
        "- Try HGBSVD first, fall back to TGK+DSTEMR (current architecture)\n",
        "\n",
    ]
