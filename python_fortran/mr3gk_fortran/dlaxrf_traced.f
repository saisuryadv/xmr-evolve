...
*     Per-tree-node NCD logging variables. The unit FIDRRF=99 is
*     opened by mr3gk_run_traced before invoking the SVD.
*
*     The reconstructed diag(T_node) is computed in QUAD precision
*     (real*16, gfortran libquadmath) to avoid catastrophic cancellation
*     when D[i] is small and E[i-1]^2 / D[i-1] is large. Quad has
*     ~33 decimal digits, vs float64's ~16; the cancellation loses
*     ~18 digits in float64 (signal lost) but only ~18 of 33 in quad
*     (~15 clean digits remain).
      INTEGER           FIDRRF
      PARAMETER       ( FIDRRF = 99 )
      DOUBLE PRECISION  DPVMAX, DPVMIN, AD, NCD_P, NCD_R
      DOUBLE PRECISION  TINY_DENOM
      PARAMETER       ( TINY_DENOM = 1.0D-16 )
      INTEGER           IT

*     Quad-precision locals for stable diag(T_node) reconstruction.
*     We INVERT the block-twisted N*G*N^T rep back to the implicit
*     tridiagonal T_node and check whether its diagonal is nearly
*     constant (Theorem 4.5 of Willems-Lang 2012).
*
*     Per dlaxrr.f line 96-165, the rep stores:
*       SREPR(JXG+i)            = G_i      (scalar pivot)
*       SREPR(IXGNSQ+i)         = e_i^2    (squared offdiag, set to UNDEF at K)
*       SREPI(JYOMGA+i)         = Omega_i  (block flag, 1 at block start)
*       Twist position K marks the join between top-down and bottom-up sweeps.
*
*     Reconstruction of T_node[i,i]:
*       Scalar position (Omega_i = 0):
*         T[i,i] = G_i + e_{i-1}^2 / G_{i-1}     (with e_0 := 0)
*       Block at (i, i+1) when Omega_i = 1:
*         T[i,i] + T[i+1,i+1] = G_i + G_{i+1}              (trace, Vieta)
*         T[i,i] * T[i+1,i+1] = G_i*G_{i+1}                (det of 2x2 minus e^2)
*         => T[i,i], T[i+1,i+1] are roots of x^2 - s*x + p = 0
*       At twist position K: skip (sentinel UNDEF) and reconstruct from
*       both sides; we report the union of recoverable entries.
*
*     IXBDET in dlaxrr.f is at offset N (i.e. SREPR(N+i)).
*     IXGNSQ is at offset 3*N+2.
      REAL(KIND=16)     QG_PREV, QG_I, QG_NEXT, QESQ, QESQ_PREV
      REAL(KIND=16)     QDIAG_I, QDIAG_IP1
      REAL(KIND=16)     QDMAX_R, QDMIN_R
      REAL(KIND=16)     QS, QP, QDISC
      REAL(KIND=16)     QABS_DMIN
      REAL(KIND=16)     QTINY
      PARAMETER       ( QTINY = 1.0E-16_16 )
      INTEGER           IT_K, IXGNSQ_TR, IXBDET_TR, JYOMGA_TR
      LOGICAL           HAVE_FIRST
*     ----------------------------------------
...
*     Init all candidates as actuve but not evaluated and unfudged.
      IWORK(IYCTWI:IYCTWI+MAXNCAND-1) = 0
      IWORK(IYCFDG:IYCFDG+MAXNCAND-1) = 0
      IWORK(IYCGRT:IYCGRT+MAXNCAND-1) = 0
*
*     The blown up father bounds are zero as long as they are not set.
      RWORK(IXBLUP + 2*ICBEG-1 : IXBLUP + 2*ICEND) = ZERO
*
...
*     -------- TRACED instrumentation: per-child NCD log line --------
*     We compute two quantities at every successful child rep:
*       NCD_pivots  = (|D|max - |D|min) / (|D|min + 1e-16)
*       NCD_recon   = (diagmax - diagmin) / (diagmin + 1e-16)
*     where diag_i is the i-th diagonal entry of T_node = L*D*L^T,
*     reconstructed from the rep:
*       T_node[i,i] = D[i] + E[i-1]^2 / D[i-1]   (i >= 2)
*       T_node[1,1] = D[1]
*     Per Theorem 4.5 of Willems-Lang 2012, the GK-rep preserves
*     near-constant-diagonal: NCD_recon should stay ~ 0 at every node.
*
*     SREPR(JXG + 1..N)         = D[1..N]              (pivot diagonal)
*     SREPR(IXGNSQ + 1..N-1)    = E[1..N-1]^2          (squared offdiag)
*     IXGNSQ in dlaxrr.f is set to (3*N+2) -- same offset here.
      IF( SUCCESS )THEN
*        --- Build N and G as explicit n x n matrices, multiply
*            T_recon = N * G * N^T, take diagonal, compute NCD.
*
*        XMR's block-twisted block-LDL^T factorization:
*          - Top sweep (rows 1..K-1): N is unit lower triangular.
*          - Bottom sweep (rows K+1..N): N is unit upper triangular.
*          - Twist row K: junction (no off-diagonal coupling to/from K).
*          - In a SCALAR region (Omega_i = 0), N has sub/super-diag
*            ell_i = e_i / G_i in the corresponding direction.
*          - In a 2x2 BLOCK at (i, i+1) (Omega_i = 1), N has identity
*            on the (i, i+1) sub-block (no off-diagonal entry inside the
*            block); G has the dense 2x2 block [[G_i, e_i], [e_i, G_{i+1}]]
*            sitting on its diagonal.
*          - Outside blocks, G is diagonal with G(i) on its diagonal.
*
*        Allocate quad-precision NMAT(N,N) and GMAT(N,N) on the heap and
*        do explicit O(n^3) matmul. For trace logging this is acceptable.
         IXGNSQ_TR = 3 * N + 2
         IXBDET_TR = N
         JYOMGA_TR = JYOMGA
         IT_K = TWIST

         CALL TRACED_DIAG_NCD( N, IT_K, JXG, JYOMGA_TR,
     $                         SREPR, SREPI, E,
     $                         QDMIN_R, QDMAX_R, NCD_R )

*        Diagnostic: count block positions in this rep
         IT_K = 0
         DO IT = 1, N
            IF( SREPI( JYOMGA_TR + IT ) .EQ. 1 ) IT_K = IT_K + 1
         ENDDO
         WRITE(FIDRRF, 8000)
     $        DEPTH, N, ICBEG, ICEND, TAU,
     $        REAL(QDMIN_R, KIND=8), REAL(QDMAX_R, KIND=8),
     $        NCD_R, IT_K, TWIST
 8000    FORMAT('NODE depth=', I0,
     $          ' n=', I0,
     $          ' icbeg=', I0,
     $          ' icend=', I0,
     $          ' tau=', ES15.8E2,
     ...
     $          ' twist=', I0)
      ENDIF
*     -------- end TRACED instrumentation --------
      END SUBROUTINE DLAXRF
*
************************************************************************
*
*     ----- Helper for traced reconstruction of T = N*G*N^T -----
*
      SUBROUTINE TRACED_DIAG_NCD( N, K, JXG, JYOMGA,
     $                            REPR, REPI, E,
     $                            QDMIN, QDMAX, NCD8 )
      IMPLICIT NONE
      INTEGER,           INTENT(IN)  :: N, K, JXG, JYOMGA
      INTEGER,           INTENT(IN)  :: REPI(*)
      DOUBLE PRECISION,  INTENT(IN)  :: REPR(*), E(*)
      REAL(KIND=16),     INTENT(OUT) :: QDMIN, QDMAX
      DOUBLE PRECISION,  INTENT(OUT) :: NCD8
*
      REAL(KIND=16), ALLOCATABLE :: NMAT(:,:), GMAT(:,:), TMP(:,:)
      REAL(KIND=16) :: TII, QABS_DMIN, QTINY, QELL
      PARAMETER     ( QTINY = 1.0E-16_16 )
      INTEGER :: I, II, JJ, KK
      LOGICAL :: HAVE_FIRST
*
      ALLOCATE( NMAT(N,N), GMAT(N,N), TMP(N,N) )
      DO JJ = 1, N
         DO II = 1, N
            NMAT(II,JJ) = 0.0E0_16
            GMAT(II,JJ) = 0.0E0_16
...
*
*     -- Build G ----
*     Walk i = 1..N skipping twist K; at scalar pos place G(i) on diag,
*     at block pos (i,i+1) place 2x2 dense block.
      I = 1
 100  IF( I .GT. N ) GOTO 200
      IF( I .EQ. K )THEN
         GMAT(I,I) = REAL( REPR(JXG + I), KIND=16 )
         I = I + 1
         GOTO 100
      ENDIF
      IF( REPI(JYOMGA + I) .EQ. 1 .AND. I .LT. N )THEN
*        2x2 block at (I, I+1): GMAT(I:I+1, I:I+1) =
*           [[G_i, e_i], [e_i, G_{i+1}]]
         GMAT(I,   I)   = REAL( REPR(JXG + I    ), KIND=16 )
         GMAT(I,   I+1) = REAL( E(I)             , KIND=16 )
         GMAT(I+1, I)   = REAL( E(I)             , KIND=16 )
         GMAT(I+1, I+1) = REAL( REPR(JXG + I + 1), KIND=16 )
         I = I + 2
      ELSE
         GMAT(I,I) = REAL( REPR(JXG + I), KIND=16 )
         I = I + 1
...
*
*     -- Build N ----
*     Top sweep (rows 1..K-1): scalar N(I+1, I) = e_I / G_I.
*     At block start (Omega_i = 1), no entry coupling i to i+1 (block
*     is dense in G, identity in N).
      DO I = 1, K - 1
         IF( REPI(JYOMGA + I) .EQ. 1 )THEN
*           start of 2x2 block at (I, I+1) — no N coupling between them
            CYCLE
         ENDIF
*        Skip if this position was the second half of a preceding block
         IF( I .GT. 1 )THEN
            IF( REPI(JYOMGA + I - 1) .EQ. 1 )THEN
*              I is second half of block at (I-1, I): the coupling I -> I+1
*              is the next block-or-scalar boundary. We model it as scalar
*              from I to I+1 with ell = e_I / G_I.
            ENDIF
         ENDIF
         IF( ABS( REPR(JXG + I) ) .GT. 0.0D0 .AND. I + 1 .LE. N )THEN
            QELL = REAL( E(I), KIND=16 ) /
     $             REAL( REPR(JXG + I), KIND=16 )
            NMAT(I+1, I) = QELL
         ENDIF
      ENDDO
*
*     Bottom sweep (rows K+1..N): scalar N(I-1, I) = e_{I-1} / G_I.
      DO I = K + 1, N
         IF( REPI(JYOMGA + I) .EQ. 1 )THEN
            CYCLE
         ENDIF
         IF( I .LT. N )THEN
            IF( REPI(JYOMGA + I + 1) .EQ. 1 )THEN
*              second half of next block — handled by block placement
            ENDIF
         ENDIF
         IF( ABS( REPR(JXG + I) ) .GT. 0.0D0 .AND. I - 1 .GE. 1 )THEN
            QELL = REAL( E(I-1), KIND=16 ) /
     $             REAL( REPR(JXG + I), KIND=16 )
            NMAT(I-1, I) = QELL
         ENDIF
      ENDDO
*
*     -- TMP = N * G  (n*n*n adds in quad) ----
      DO JJ = 1, N
         DO II = 1, N
            TII = 0.0E0_16
            DO KK = 1, N
               TII = TII + NMAT(II, KK) * GMAT(KK, JJ)
            ENDDO
            TMP(II, JJ) = TII
         ENDDO
      ENDDO
*     -- T_recon[i,i] = (TMP * N^T)[i,i] = sum_k TMP(i,k) * NMAT(i,k) ----
*     Track min/max over diagonal only; full matrix not needed.
      HAVE_FIRST = .FALSE.
      QDMIN = 0.0E0_16
      QDMAX = 0.0E0_16
      DO I = 1, N
         ...
         IF( .NOT. HAVE_FIRST )THEN
            ...
            HAVE_FIRST = .TRUE.
         ELSE
            IF( TII .GT. QDMAX ) QDMAX = TII
            IF( TII .LT. QDMIN ) QDMIN = TII
         ENDIF
      ENDDO
      QABS_DMIN = ABS( QDMIN )
      IF( QABS_DMIN .GT. QTINY )THEN
         NCD8 = REAL( ( QDMAX - QDMIN ) / QABS_DMIN, KIND=8 )
      ELSE
         NCD8 = REAL( ( QDMAX - QDMIN ) / ( QABS_DMIN + QTINY ),
     $                KIND=8 )
      ENDIF
      DEALLOCATE( NMAT, GMAT, TMP )
      END SUBROUTINE TRACED_DIAG_NCD
*
************************************************************************
