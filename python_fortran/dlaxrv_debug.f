...
*
      ...
      ELSE
         ...
         ENDDO
         RWORK(IXSHFP+N:IXSHFP+2*N-1) = ONE
...
*
      ...
      RWORK(IXROOT : IXROOT+RSTRIDER-1) = ROOTR
      IWORK(JXROOT : JXROOT+RSTRIDEI-1) = ROOTI
...
*     will update it automatically to at lest |ew|.
      ...
      DO
         ...
         CALL DBG_NODE(DEPTH, IL, IU, TAUBAR)
...
*           Record bounds at time of classification
            RWORK(IXGBND + 2*IL : IXGBND + 2*IU-1)
     $        = EWL_LU(2*IL : 2*IU-1)
...
*
     ...
     $              )
               ...
               W(I) = LAMBDA + TAUBAR
               CALL DBG_SINGLETON(DEPTH,I,LAMBDA,TAUBAR,W(I))
...
*                    Could not find a child representation
                     STATUS = -2
                     CALL DBG_CLUSTER_FAIL(DEPTH,I,J,XRFSTA)
                  ELSE
                     DO K = 0, 2*(J-I+1)-1
                        EWL_LU(2*I-1 + K) = RWORK(IXCEWB + K)
                        EWL_AE(2*I-1 + K) = IWORK(JXCEWB + K)
                     ENDDO
                     CALL DBG_CLUSTER(DEPTH,I,J,TAU,TAUBAR+TAU)
                  ENDIF
               ENDIF


               IF( STATUS .NE. 0 )THEN
                  ISUPPZ(2*WI:2*WJ:2) = STATUS
                  Z(1:N,WI:WJ) = ZERO
                  W(WI:WJ) = (EWL_LU(2*I-1) + EWL_LU(2*J))/2 + TAUBAR
               ELSE
                  DEPTH = DEPTH+1
