...
C        CALL DBG_NODE(DEPTH, IL, IU, TAUBAR)
...
*
     ...
     $              )
               ...
               W(I) = LAMBDA + TAUBAR
C              CALL DBG_SINGLETON(DEPTH, I, LAMBDA, TAUBAR, W(I))
...
*                    Could not find a child representation
                     STATUS = -2
C                    CALL DBG_CLUSTER_FAIL(DEPTH, I, J, XRFSTA)
                  ELSE
                     DO K = 0, 2*(J-I+1)-1
                        EWL_LU(2*I-1 + K) = RWORK(IXCEWB + K)
                        EWL_AE(2*I-1 + K) = IWORK(JXCEWB + K)
                     ENDDO
C                    CALL DBG_CLUSTER(DEPTH, I, J, TAU, TAUBAR+TAU)
                  ENDIF
               ENDIF
