...
inline void postprocess_tgk_svd(int n, const double* d, const double* e,
    ...
    if (n >= 2 && n <= 80) {
        // Quick orthogonality check: sample a few adjacent pairs
        double ortho_v = 0.0, ortho_u = 0.0;
        int ncheck = std::min(n - 1, 8);
        ...
        double ortho_thresh = 0.01;  // Above this, vectors are not orthogonal

        if (ortho_v > ortho_thresh && ortho_u < ortho_thresh) {
            // V is bad, U is good → recover V = B^T*U/σ for each column
            for (int j = 0; j < n; j++) {
        ...
        } else if (ortho_u > ortho_thresh && ortho_v < ortho_thresh) {
            // U is bad, V is good → recover U = B*V/σ for each column
            for (int j = 0; j < n; j++) {
                if (result.sigma[j] < SAFMIN) continue;
