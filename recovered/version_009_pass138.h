...
inline void postprocess_tgk_svd(int n, const double* d, const double* e,
    ...
    // Step 2: Selective bidirectional one-sided recovery
    // Only trigger when one side is dramatically better than the other (100x ratio)
    double sigma_max = result.sigma.empty() ? 0.0 : result.sigma[0];
    {
        std::vector<double> bv(n), btu(n);
        double base_thresh = (double)n * EPS * sigma_max;
        ...
        for (int j = 0; j < n; j++) {
            ...
            // Only recover when ratio is > 100 and bad side exceeds threshold
            if (res_u > 100.0 * base_thresh && res_u > 100.0 * res_v && res_v < base_thresh) {
            ...
            } else if (res_v > 100.0 * base_thresh && res_v > 100.0 * res_u && res_u < base_thresh) {
