...
// Self-contained version (inlined common TGK utilities)
...
// With GK-form: eigenvalues come in ±σ pairs, exploit pairing for better orthogonality
inline void extract_svd_from_tgk(int n, int neig, const double* W, const double* Z, int ldz,
    ...
    for (int j = 0; j < n; j++) {
        // Take from the positive end (largest eigenvalue first)
...
inline BidiagSVDResult bidiag_svd(int n, const double* d, const double* e) {
    ...
    // Workspace query
    // Request all eigenvalues for now (debug: partial request returned zeros)
    int wil = 1, wiu = m2n;
    ...
    int neig = wiu - wil + 1;  // number of eigenvalues requested
    int ldz = m2n;
