...
C        Support for GK-type matrices re-enabled for bidiagonal SVD.
C        For TGK matrices: DMAX=DMIN=0, so this condition is trivially
C        satisfied. TAU=0 (no initial outside shift), TYPE=1 with
C        alternating OMEGA creates 2x2 blocks preserving GK structure.
*        -----------------------------------
