...
inline void postprocess_tgk_svd(int n, const double* d, const double* e,
    ...
    // Use V*V^T metric (matching evaluate's ||I - Q*Q^T||_max) for detection
    double sigma_max = result.sigma.empty() ? 0.0 : result.sigma[0];
    if (n >= 2 && n <= 50) {
        // Compute max |V*V^T - I| and max |U*U^T - I| (O(n³), gated to n≤50)
        double ortho_v = 0.0, ortho_u = 0.0;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j <= i; j++) {
                double vvt = 0.0, uut = 0.0;
                for (int k = 0; k < n; k++) {
                    vvt += result.V[k * n + i] * result.V[k * n + j];
                    uut += result.U[k * n + i] * result.U[k * n + j];
                }
                double target = (i == j) ? 1.0 : 0.0;
                ortho_v = std::max(ortho_v, std::abs(vvt - target));
                ortho_u = std::max(ortho_u, std::abs(uut - target));
            }
        }
        ...
        if (ortho_v > ortho_thresh && ortho_u < ortho_v * 0.01) {
            // V is bad, U is much better → recover V = B^T*U/σ
            for (int j = 0; j < n; j++) {
            ...
            // U is bad, V is much better → recover U = B*V/σ
            for (int j = 0; j < n; j++) {
                if (result.sigma[j] < SAFMIN) continue;
