...
inline void postprocess_tgk_svd(int n, const double* d, const double* e,
    ...
    // Step 2: Bidirectional one-sided recovery
    // For each singular triplet, check which side (U or V) is better
    // and recover the other from it
    ...
    for (int j = 0; j < n; j++) {
        ...
        // Compute B*v and B^T*u
        std::vector<double> bv(n), btu(n);
        bidiag_multiply(n, d, e, v_col, bv.data());
        bidiag_transpose_multiply(n, d, e, u_col, btu.data());

        // Residuals: ||Bv - σu|| and ||B^Tu - σv||
        double res_u = 0.0, res_v = 0.0;
        for (int i = 0; i < n; i++) {
            double du = bv[i] - result.sigma[j] * u_col[i];
            double dv = btu[i] - result.sigma[j] * v_col[i];
            res_u += du * du;
            res_v += dv * dv;
        }
        res_u = std::sqrt(res_u);
        res_v = std::sqrt(res_v);

        // Threshold: expected residual of a good vector is O(n * eps * ||B||)
        double rel_thresh = (double)n * EPS * sigma_max;

        if (res_u > rel_thresh && res_v < rel_thresh) {
            // V is good, recover U = B*v / σ
            for (int i = 0; i < n; i++) u_col[i] = bv[i] / result.sigma[j];
            double u_norm = dnrm2_(&n, u_col, &one);
            if (u_norm > SAFMIN) {
                double inv = 1.0 / u_norm;
                dscal_(&n, &inv, u_col, &one);
            }
        } else if (res_v > rel_thresh && res_u < rel_thresh) {
            // U is good, recover V = B^T*u / σ
            for (int i = 0; i < n; i++) v_col[i] = btu[i] / result.sigma[j];
            double v_norm = dnrm2_(&n, v_col, &one);
            if (v_norm > SAFMIN) {
                double inv = 1.0 / v_norm;
                dscal_(&n, &inv, v_col, &one);
            }
        } else if (res_u > rel_thresh && res_v > rel_thresh) {
            // Both bad — try recovering from whichever has smaller residual
            if (res_v <= res_u) {
                // V is slightly better, recover U
                for (int i = 0; i < n; i++) u_col[i] = bv[i] / result.sigma[j];
                double u_norm = dnrm2_(&n, u_col, &one);
                if (u_norm > SAFMIN) {
                    double inv = 1.0 / u_norm;
                ...
                // U is slightly better, recover V
                for (int i = 0; i < n; i++) v_col[i] = btu[i] / result.sigma[j];
                double v_norm = dnrm2_(&n, v_col, &one);
                if (v_norm > SAFMIN) {
                    double inv = 1.0 / v_norm;
    ...
    // Step 4: Two-pass selective reorthogonalization (shifted chunked MGS, O(n²))
    // Pass 1: standard chunks, Pass 2: shifted by half-chunk for cross-chunk ortho
    double cluster_tol = 1000.0 * EPS * sigma_max;
    ...
    int MAX_CHUNK = 64;
    // Two passes: pass 0 starts at cstart, pass 1 starts at cstart + MAX_CHUNK/2
    for (int pass = 0; pass < 2; pass++) {
        for (size_t cl = 0; cl < cluster_start.size(); cl++) {
            int cstart = cluster_start[cl];
            int cend = cluster_end[cl];
            int offset = (pass == 0) ? 0 : MAX_CHUNK / 2;

            for (int chunk_start = cstart + offset; chunk_start <= cend; chunk_start += MAX_CHUNK) {
                int chunk_end = std::min(chunk_start + MAX_CHUNK - 1, cend);
                int chunk_begin = std::max(chunk_start, cstart);

                for (int j = chunk_begin; j <= chunk_end; j++) {
                    double* vj = &result.V[j * n];
                    for (int k = chunk_begin; k < j; k++) {
