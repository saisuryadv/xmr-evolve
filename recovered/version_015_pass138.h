...
inline void postprocess_tgk_svd(int n, const double* d, const double* e,
    ...
    // Step 2: Orthogonality-guided one-sided recovery
    // Check which side (U or V) has better orthogonality, then recover the other
    double sigma_max = result.sigma.empty() ? 0.0 : result.sigma[0];
    if (n >= 2 && n <= 200) {
        double ortho_v = 0.0, ortho_u = 0.0;
        if (n <= 50) {
            // Full pairwise check for small n — O(n³) but n≤50 so fast
            for (int j = 0; j < n; j++) {
                for (int k = j + 1; k < n; k++) {
                    double dv = std::abs(ddot_(&n, &result.V[j * n], &one, &result.V[k * n], &one));
                    double du = std::abs(ddot_(&n, &result.U[j * n], &one, &result.U[k * n], &one));
                    ortho_v = std::max(ortho_v, dv);
                    ortho_u = std::max(ortho_u, du);
                }
            }
        } else {
            // Sampled check for larger n — adjacent + spread pairs
            for (int j = 0; j < std::min(n - 1, 16); j++) {
                double dv = std::abs(ddot_(&n, &result.V[j * n], &one, &result.V[(j+1) * n], &one));
                double du = std::abs(ddot_(&n, &result.U[j * n], &one, &result.U[(j+1) * n], &one));
                ortho_v = std::max(ortho_v, dv);
                ortho_u = std::max(ortho_u, du);
            }
            // Also check some spread-out pairs
            for (int j = 0; j < std::min(n / 2, 8); j++) {
                int k = n - 1 - j;
                if (k <= j) break;
                double dv = std::abs(ddot_(&n, &result.V[j * n], &one, &result.V[k * n], &one));
                double du = std::abs(ddot_(&n, &result.U[j * n], &one, &result.U[k * n], &one));
        ...
        double ortho_thresh = 0.01;

        if (ortho_v > ortho_thresh && ortho_u < ortho_v * 0.01) {
            // V is bad, U is much better → recover V = B^T*U/σ for each column
            for (int j = 0; j < n; j++) {
                if (result.sigma[j] < SAFMIN) continue;
                std::vector<double> btu(n);
                bidiag_transpose_multiply(n, d, e, &result.U[j * n], btu.data());
                for (int i = 0; i < n; i++) result.V[j * n + i] = btu[i] / result.sigma[j];
                double v_norm = dnrm2_(&n, &result.V[j * n], &one);
                if (v_norm > SAFMIN) {
                    double inv = 1.0 / v_norm;
                    dscal_(&n, &inv, &result.V[j * n], &one);
                }
            }
        } else if (ortho_u > ortho_thresh && ortho_v < ortho_u * 0.01) {
            // U is bad, V is much better → recover U = B*V/σ for each column
            for (int j = 0; j < n; j++) {
                if (result.sigma[j] < SAFMIN) continue;
                std::vector<double> bv(n);
                bidiag_multiply(n, d, e, &result.V[j * n], bv.data());
                for (int i = 0; i < n; i++) result.U[j * n + i] = bv[i] / result.sigma[j];
                double u_norm = dnrm2_(&n, &result.U[j * n], &one);
                if (u_norm > SAFMIN) {
                    double inv = 1.0 / u_norm;
                    dscal_(&n, &inv, &result.U[j * n], &one);
                }
            }
        }
    }
